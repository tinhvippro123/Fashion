...
public class OrderServiceImpl implements OrderService {
	...
	@Override
	public Order placeOrder(Long userId, String sessionId, PlaceOrderRequest request) {
...
//				throw new RuntimeException("The product " + productName + " (" + colorName + " - " + sizeName + ") does not have sufficient stock");
				throw new RuntimeException("The product " + product.getName() + " (" + color.getName() + " - " + size.getName()	 + ") does not have sufficient stock");
			}
...
//			Lấy giá, ưu tiên lấy giá của variant, nếu không có(null) thì sẽ lấy giá gốc là giá bên product
			double price = variant.getId() != null ? variant.getPrice() : variant.getProductColor().getProduct().getBasePrice();
			
//			Tìm ảnh main để hiển thị trong order
			String imageUrl = variant.getProductColor().getImages().stream().filter(img -> img.getImageType() == ImageType.MAIN).findFirst().map(ProductImage::getImageUrl).orElse(null);
