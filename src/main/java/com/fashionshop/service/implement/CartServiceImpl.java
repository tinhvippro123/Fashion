...
//		Tìm Variant
		ProductVaraint variant = variantRepository.findById(request.getVariantId()).orElseThrow(()-> new RuntimeException("The product does not exist."));
		
//		Kiểm tra sản phẩm đã có trong giỏ chưa, nếu có rồi thì cộng số lượng
		Optional<CartItem> existingItem  = cart.getCartItems().stream().filter(item -> item.getVariant().getId().equals(variant.getId())).findFirst();
...
//			Tìm ảnh MAIN trong nhóm màu đóa
			...
			itemResponses.add(CartResponse.CartItemResponse.builder()
					...
					.colorName(v.getProductColor().getColor().getName())
...
//		Check xem item có thuộc giỏ hàng không, để tranh hacker xóa giỏ người khác ấy mà
		Optional<CartItem> itemToDelete = cart.getCartItems().stream().filter(item -> item.getId().equals(cartItemId)).findFirst();
...
//			Xóa luôn khỏi database
	...
	@Override
	@Transactional
	public CartResponse updateItemQuantity(Long userId, String sessionId, Long itemId, Integer newQuantity) {
		Cart cart = getOrCreateCart(userId, sessionId);
		
		Optional<CartItem> itemOptional = cart.getCartItems().stream().filter(item -> item.getId().equals(itemId)).findFirst();
